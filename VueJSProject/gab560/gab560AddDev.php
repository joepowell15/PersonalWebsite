...
$sql =$conn->prepare("INSERT INTO GAB560Devices (`Serial_Number`, `UNT_Tracking_Number`, `U_Height`, `Model_Manufacturer`, `Device_Name`, `Owner`, `Grp`, `Comment`) VALUES (?,?,?,?,?,?,?,?)");	
$sql->bind_param("ssisssss",$serial,$unt,$height,$model,$dName,$owner,$group,$dComm);
...
$sql =$conn2->prepare("INSERT INTO GAB560DeviceInRack (`Location`, `Serial_Number`, `Position`) VALUES (?,?,?)");	
...
echo"<script>window.location.assign(\"/VueJSProject/gab560/add.php?\")</script>";
}
else {

	echo"<script>window.location.assign(\"/VueJSProject/gab560/add.php?message=Bad\")</script>";
}
