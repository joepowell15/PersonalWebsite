...
$sql =$conn->prepare("INSERT INTO DiscoDevices (`Serial_Number`, `UNT_Tracking_Number`, `U_Height`, `Manufacturer`,`Model`, `Device_Name`, `Owner`, `Grp`, `Comment`) VALUES (?,?,?,?,?,?,?,?,?)");	
...
$sql =$conn2->prepare("INSERT INTO DiscoDeviceInRack (`Location`, `Serial_Number`, `Position`) VALUES (?,?,?)");	
...
echo"<script>window.location.assign(\"/VueJSProject/discoPark/add.php?\")</script>";
}
else {
echo "
	echo"<script>window.location.assign(\"/VueJSProject/discoPark/add.php?message=Bad\")</script>";
}
